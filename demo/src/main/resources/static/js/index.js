...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function typeWriter(element, text, speed = 100) {
        ...
        function type() {
            if (i < text.length) {
                ...
                // Remover cursor piscante após terminar de digitar
                setTimeout(() => {
    ...
    if (typingTitle) {
        ...
        setTimeout(() => {
            typeWriter(typingTitle, originalText, 80);
        }, 500);
    }
    ...
    function initScrollSpy() {
        ...
        highlightNavLink(); // Executar uma vez ao carregar
    }
    initScrollSpy();
    ...
    function initScrollAnimations() {
        ...
        // Observer para elementos com fade-in
        ...
        // Observer para feature cards com staggered animation
        const featureObserver = new IntersectionObserver((entries) => {
            entries.forEach((entry) => {
                if (entry.isIntersecting) {
                    const delay = entry.target.getAttribute("data-delay") || 0;
                    setTimeout(() => {
                        entry.target.querySelector(".feature-card").classList.add("visible");
                    }, delay);
        ...
        // Observer para step items com slide animation
    ...
    function initSmoothScroll() {
        document.querySelectorAll('a[href^="#"]').forEach(anchor => {
            anchor.addEventListener('click', function (e) {
                ...
                if (target) {
                    window.scrollTo({
                        top: target.offsetTop - 80, // Ajuste para compensar a navbar fixa
                        behavior: 'smooth'
                    });
                }
            });
    ...
    // Botão de voltar ao topo com aparecimento suave
    ...
    class AssistenteVirtual {
        ...
        setupEventListeners() {
            ...
            document.addEventListener("click", (e) => {
                if (this.isOpen && !this.chatPanel.contains(e.target) && !this.chatIcon.contains(e.target)) {
                    this.closeChat()
                }
            })
        ...
        addMessage(message, sender = "user") {
            ...
            messageDiv.className = `chat-message ${sender === "user" ? "message-sent" : "message-received"}`
        ...
        showMenu(menuId, addTitle = true) {
            ...
            if (optionsCount <= 2) {
                ...
                buttonStyle =
                    "display: flex; flex-direction: row; justify-content: flex-start; align-items: center; gap: 0.5rem; padding: 0.4rem 0.75rem;"
            ...
            menu.options.forEach((option) => {
                ...
                if (optionsCount <= 2) {
                    button.innerHTML = `<i class="bi ${option.icon}"></i> <span>${option.text}</span>`
                } else {
                    button.innerHTML = `<i class="bi ${option.icon}"></i><span>${option.text}</span>`
                }
